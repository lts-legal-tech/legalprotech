...
import { Download, ExternalLink, Loader2, RefreshCw, Video } from 'lucide-react';
...
export function VideoLibraryWorkspace() {
  ...
  async function load() {
    ...
    try {
      ...
      setError(err.message || 'Không tải được kho video.');
    } finally {
      setLoading(false);
    }
  }
    ...
    <div className="space-y-5">
      <section className="panel rounded-[30px] p-5 md:p-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div>
            <div className="glass-chip">legalprotech · Video storage</div>
            <h1 className="mt-4 text-3xl font-black tracking-[-0.04em] text-slate-950 md:text-4xl">Kho video của tôi</h1>
            <p className="mt-3 max-w-3xl text-sm leading-7 text-slate-600">
              Trang này thay cho My Product. Video được lọc theo <b>user_id</b> của tài khoản đang đăng nhập, nên tài khoản khác không thấy nhầm kết quả của bạn.
            </p>
            <div className="mt-3 text-xs text-slate-400">User ID: {userId || 'chưa xác định'} · {total} video</div>
          </div>
      ...
      {loading ? (
        <div className="panel flex min-h-[420px] items-center justify-center rounded-[30px] p-6 text-sm font-medium text-slate-600">
          <Loader2 className="mr-2 h-5 w-5 animate-spin" /> Đang tải kho video...
        </div>
      ) : !items.length ? (
        <div className="flex min-h-[420px] flex-col items-center justify-center rounded-[30px] border border-dashed border-slate-200 bg-white px-6 text-center">
          ...
          <div className="mt-4 text-xl font-semibold text-slate-900">Chưa có video nào</div>
          <div className="mt-2 max-w-md text-sm leading-7 text-slate-500">Sau khi tạo video bằng Text to Video hoặc Image to Video, kết quả hoàn tất sẽ tự xuất hiện tại đây theo user_id của bạn.</div>
        </div>
      ) : (
        <div className="grid gap-4 md:grid-cols-2 2xl:grid-cols-3">
          {items.map((item) => (
            <article key={`${item.jobId}-${item.id}`} className="panel overflow-hidden rounded-[28px] p-3">
              ...
              <div className="p-3">
                <div className="font-semibold text-slate-900">{item.title || 'Video'}</div>
